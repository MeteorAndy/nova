...
| 33 | Per-session draft/scroll/run state | ✅ | per-session composer draft (AgentPanel.test.tsx), per-session scroll capture/restore (AgentMessageList.test.tsx), and run-state resume (lifecycle tests) verified |
...
| 91 | No submit/switch/migration while offline | ✅ | offline switch, Agent composer submit, story-stage submit, and structure migration gates verified (App integration + InputArea + StoryStage tests) |
